class Admin::ReviewSocialMediaController < Admin::AdminController
  ...
  def index
    if params[:status] == "archived"
      @outlets = Outlet.joins(:users).where("users.email" =>current_user.email, "outlets.draft_id" => nil, "outlets.status" => 2).order("outlets.validated_at ASC").page(current_page).per(10)
    elsif params[:status] == "published"
      if params[:review] == "needs"
        @outlets = Outlet.joins(:users).where("users.email" =>current_user.email, "outlets.draft_id" => nil, "outlets.status" => 1).where("outlets.validated_at <= ?",180.days.ago).order("outlets.validated_at ASC").page(current_page).per(10)
      else
        @outlets = Outlet.joins(:users).where("users.email" =>current_user.email, "outlets.draft_id" => nil, "outlets.status" => 1).order("outlets.validated_at ASC").page(current_page).per(10)
      end
    else
      @outlets = Outlet.joins(:users).where("users.email" =>current_user.email, "outlets.draft_id" => nil).order("outlets.validated_at ASC").page(current_page).per(10)
